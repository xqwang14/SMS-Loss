...
from avion.data.clip_dataset import VideoCaptionDatasetCLIP, collect_relevancy
from avion.data.tokenizer import tokenize
from avion.data.transforms import Permute

from avion.losses.losses import ClipLoss, MaxMarginRankingLoss, AdaptiveMaxMarginRankingLoss, SymmetricMultiSimiliarityLoss
...
def get_args_parser():
    ...
    parser.add_argument('--relevancy-train', type=str,
                        default='datasets/EK100/epic-kitchens-100-annotations/retrieval_annotations/relevancy/caption_relevancy_EPIC_100_retrieval_train.pkl')
    parser.add_argument('--relevancy-test', type=str,
    ...
    # old model settings
    parser.add_argument('--model', default='CLIP_VITB16', choices=['CLIP_VITB16', 'CLIP_VITL14', 'CLIP_VITL14_336PX'], type=str)
    ...
    parser.add_argument('--pretrain-path', default=None, type=str, help='path of pretrained model')
    #new model settings
    ...
    # sms loss
    parser.add_argument('--loss-margin', default=0.6, type=float)
    parser.add_argument('--loss-thres', default=0.1, type=float)
    # clip loss
...
def main(args):
    ...
    # ).cuda(args.gpu)
    correlation_matrix = pickle.load(open(args.relevancy_train, 'rb'))

    criterion = SymmetricMultiSimiliarityLoss(
        margin = args.loss_margin,
        thres = args.loss_thres,
    ...
    train_dataset = VideoCaptionDatasetCLIP(
        args=args, metadata=args.train_metadata,
    ...
    val_dataset = VideoCaptionDatasetCLIP(
        args=args, metadata= args.val_metadata,
...
def train(train_loader, transform_gpu, model, criterion, optimizer, scaler, epoch, lr_schedule, args):
    ...
    for data_iter, inputs in enumerate(train_loader):
        ...
        relevancy_col = inputs.pop().unsqueeze(0)
        relevancy_row = inputs.pop().unsqueeze(0)
        relevancy_weight = torch.cat((relevancy_col, relevancy_row), 0)

        # for i, x in enumerate(relevancy_row):
        #     for j, y in enumerate(relevancy_col):
        #         relevancy_weight[i][j] = torch.tensor(train_dataset.return_relevancy_mat(x,y))

        relevancies = inputs.pop()  # loader will a "relevancy" variable; we need it for ek100_mir
        # print(relevancies)
        # print(relevancies.size()) torch.size([batchsize])
        optimizer.zero_grad()
...
def validate_mir(val_loader, transform_gpu, model, criterion, args):
    ...
    with amp.autocast(enabled=not args.disable_amp):
        with torch.no_grad():
            ...
            for i, inputs in enumerate(val_loader):
                ...
                inputs = [tensor.cuda(args.gpu, non_blocking=True) for tensor in inputs]
                #pop out the correlation matrix
                inputs.pop()
                inputs.pop()
    ...
    rel_matrix = pd.read_pickle(args.relevancy_test)
