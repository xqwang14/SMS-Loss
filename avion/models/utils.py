...
def remap_keys_from_open_clip_to_vit(
    ...
    for layer in range(visual_transformer_layers):
        ...
        if use_flash_attn and rope:
            for src_name, tgt_name in {
                'attn.in_proj_weight': 'Wqkv.weight', 'attn.in_proj_bias': 'Wqkv.bias',
                'attn.out_proj.weight': 'out_proj.weight', 'attn.out_proj.bias': 'out_proj.bias',
        ...
        # if rope:
        #     for src_name, tgt_name in {
        #         'attn.Wqkv.weight': 'Wqkv.weight', 'attn.Wqkv.bias': 'Wqkv.bias',
        #         'attn.out_proj.weight': 'out_proj.weight', 'attn.out_proj.bias': 'out_proj.bias',
        #     }.items():
        #         key_mapping[f"visual.transformer.resblocks.{layer}.{src_name}"] = f"visual.transformer.resblocks.{layer}.{tgt_name}"
...
def remap_state_dict_keys(use_fast_conv1, state_dict):
    new_state_dict = {}
    for key in state_dict.keys():
        new_key = key
        # Add "trunk" after "visual."
        if key.startswith("visual."):
            new_key = key.replace("visual.trunk.", "visual.")
            if key.startswith("visual.trunk.patch_embed.proj"):
                new_key = key.replace("visual.trunk.patch_embed.proj.", "visual.conv1.")
            if key.startswith("visual.trunk.pos_embed"):
                state_dict[key] = state_dict[key].squeeze(0)
        # Replace "text." with "textual."
        elif key.startswith("text."):
            new_key = key.replace("text.", "textual.")
        new_state_dict[new_key] = state_dict[key]
        if use_fast_conv1:
            new_state_dict['visual.conv1.weight'] = state_dict['visual.trunk.patch_embed.proj.weight'].flatten(1)
    return new_state_dict